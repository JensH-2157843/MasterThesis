...
def SaveCreatedImage(listOfImages, subPath, imageName, className):
    for i in range(len(listOfImages)):
        imagePath = f"{outDir}/{subPath}/{className}/{imageName}{'' if i == 0 else i}.jpg"
    ...
    if not FoundSubpaths:
        ...
        for i in range(len(images)):
            ...
            if(i < split_1):
                ...
                subPath = "valid"
            imagePath = f"{outDir}/{subPath}/{className}/{labels[i]}{'' if i == 0 else i}.jpg"
            os.makedirs(os.path.dirname(imagePath), exist_ok=True)
...
def masterFolderConverterV2(inputFolderPath):
...
masterFolderConverterV2("Countries")